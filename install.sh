...
# =============================================
# prettier — JSON/JS/CSS/HTML formatter (for nvim conform.nvim)
# =============================================
install_prettier() {
  echo "=== Installing prettier ==="
  if is_installed prettier; then
    echo "  Already installed: prettier $(prettier --version 2>/dev/null)"
    return
  fi
  if is_installed npm; then
    echo "  Installing prettier via npm (timeout: 120s)..."
    if ! run_with_timeout 120 npm install -g --no-audit --no-fund prettier; then
      ret=$?
      if [ $ret -eq 124 ]; then
        echo "  [!] npm install timed out after 120s."
        echo "  Try manually later: npm install -g prettier"
      else
        echo "  npm global install failed. Retrying with user prefix: $HOME/.local"
        npm config set prefix "$HOME/.local"
        if ! run_with_timeout 120 npm install -g --no-audit --no-fund prettier; then
          echo "  [!] prettier install failed. Skipping."
          record_failure "install_prettier"
        fi
        case ":$PATH:" in
        *":$HOME/.local/bin:"*) ;;
        *) export PATH="$HOME/.local/bin:$PATH" ;;
    ...
    echo "  [!] npm not found, skipping prettier. Install Node.js first."
    record_failure "install_prettier"
  fi
}
